...
// =============================================================================
// AuthForms — reusable sign-in and sign-up form components
//
// Extracted so both AuthModal and the dedicated /login and /signup pages can
// share the same form logic without duplication.
// =============================================================================
...
export function EmailField({
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <label
        htmlFor="auth-email"
        className="font-mono text-xs uppercase tracking-widest text-surface-muted"
      >
        Email
      </label>
      <input
...
export function ErrorMessage({ message }: { message: string }) {
  return (
    <p
      role="alert"
      className="rounded-lg border border-red-500/30 bg-red-500/10 px-3 py-2 font-mono text-xs text-red-400"
    >
      {message}
    </p>
  );
...
interface SignInFormProps {
  ...
  /** Show "Forgot password?" link — enabled on the dedicated /login page */
        ...
        {showForgotPassword && (
          <div className="flex justify-end">
            <Link
              ...
              className="font-mono text-[10px] text-surface-muted underline-offset-2 hover:text-tingle-aqua hover:underline"
            >
              Forgot password?
            </Link>
          </div>
...
interface SignUpFormProps {
  ...
  signUpWithEmail: (
    email: string,
    password: string,
  ) => Promise<{ error: { message: string } | null; needsConfirmation: boolean }>;
  signInWithGoogle: () => Promise<void>;
}
